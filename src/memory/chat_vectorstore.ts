...
export interface ChatVectorHit {
  id: string;
  score: number;
}

export class ChatVectorStore {
  ...
  async add(params: { id: string; text: string }): Promise<boolean> {
    ...
    db.prepare(
      `
        INSERT OR REPLACE INTO chat_embeddings (message_id, embedding, created_at)
        VALUES (?, ?, datetime('now'))
      `
  ...
  async query(text: string, limit = 5): Promise<ChatVectorHit[]> {
    if (!this.enabled) {
      return [];
    }
    ...
    const db = openDatabase();
    const rows = db
      .prepare(`SELECT message_id as id, embedding FROM chat_embeddings`)
